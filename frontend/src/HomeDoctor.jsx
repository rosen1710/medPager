...
export async function clientLoader({params}) {
    const { data: page } = await axios.get(`http://192.168.5.86:5000//get_page_by_doc/${params.id}`);
...
const Availability = async (available, id) => {
    if(available) {
        const result = await axios.post(`http://172.25.208.1:5000/available/${id}`);
    }
    else {
        const result = await axios.post(`http://172.25.208.1:5000/unavailable/${id}`);
    }
}
